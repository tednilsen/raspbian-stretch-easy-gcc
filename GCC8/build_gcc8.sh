...
sudo apt-get install libmpc-dev libmpfr-dev libgmp-dev dialog xmlstarlet python-pyudev checkinstall xa65
...
# we will use /usr/local/gcc-8.2.0 as the destination for the installed GCC 8.
# Because we want to be able to use GCC 8 while keeping GCC 6.3, we will append -8.2.0 to the resulting executables.

mkdir build && cd build
../configure -v --enable-languages=c,c++ --prefix=/usr/local/gcc-8.2.0 --program-suffix=-8.2.0 --with-arch=armv8-a+crc --with-tune=cortex-a53 --with-fpu=neon-fp-armv8 --with-float=hard --build=arm-linux-gnueabihf --host=arm-linux-gnueabihf --target=arm-linux-gnueabihf
make -j3
...
# At this point, you should be able to compile any C11 or C++17 code using gcc-8.2.0 or g++-8.2.0.
...
# 1 g++-8.2.0 -std=c++17 -Wall -pedantic -O2 your_program_name.cpp -o your_program_name

# If, at some point in the future, you’ll want to get rid of GCC 8 from your system,
# all you have to do is to remove the gcc-8.2.0 folder from /usr/local, example:
# 1 sudo rm -rf /usr/local/gcc-8.2.0
